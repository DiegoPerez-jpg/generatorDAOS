...
public Movimientostock findById(int id) {
    ...
    try (Connection conn = Conexion.getConnection();
        ...
        if (rs.next()) {
            return new Movimientostock(rs.getInt("id"), rs.getInt("fk_id_producto"), rs.getLocalDate("fecha"), rs.getDouble("cantidad"), rs.getString("motivo"), rs.getString("tipo"));
        }
    } catch (SQLException e) {
        e.printStackTrace();
    }
...
String baseSql = "SELECT * FROM movimientostock";
...
try (Connection conn = Conexion.getConnection();
            ...
            while (rs.next()) {
                lista.add(new Movimientostock(rs.getInt("id"), rs.getInt("fk_id_producto"), rs.getLocalDate("fecha"), rs.getDouble("cantidad"), rs.getString("motivo"), rs.getString("tipo")
));
}
} catch (SQLException e) {
            e.printStackTrace();
